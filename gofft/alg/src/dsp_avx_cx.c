// code for radix-RADIX/2 with single frequency
void GOERTZEL_AVX(double *data, long data_len, double k, double *out)
{
    double omega = 2.0*M_PI*RADIX/2*k/data_len;
    double sw = sin(omega);
    double cw = cos(omega);
    ...
    for (i = 0; i < data_len*2/step*step; i += step)
    {
        #pragma GCC unroll 8
        for (int m = 0; m < RADIX/4; m++)
        {
    ...
    for (; i < data_len*2; i += RADIX)
        for (int m = 0; m < RADIX/4; m++)
        {
            // zero-pad values in range data_len/4*4:data_len
            data4 = _mm256_setzero_pd();
            for (long int j = 0; j < MAX(0, MIN(4, data_len*2-i-4*m)); j++)
